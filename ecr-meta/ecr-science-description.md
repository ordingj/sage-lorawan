...
Do not apply the deployment until version `0.2.0` has been reviewed and published. Two
Kubernetes secrets are required:

- `ihv-cenic-chirpstack-api`, key `api-key`: a dedicated tenant-scoped ChirpStack v4 API key,
  created for Tracker and stored in a protected local file after review;
- `django-token`, key `token`: H02A's Sage node-auth token, generated by a Sage administrator.

The existing development API key was used only for the read-only canary and has not been copied
to H02A. The `ordingj` Sage account can sign in to the portal but cannot access the Django
node-token administration page, and H02A does not currently contain `django-token`. A Sage
administrator must supply that one credential. Keep both values out of Git, shell arguments,
and command history; create the Kubernetes secrets from protected files.
